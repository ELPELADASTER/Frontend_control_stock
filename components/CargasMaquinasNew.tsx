...
import { Calendar, Package, TrendingDown, History, Building2, Users, AlertTriangle, Search, RotateCcw, Plus, Trash2 } from 'lucide-react';
...
const CargasMaquinas: React.FC<CargasMaquinasProps> = ({ empresaInicial = 'Telecom' }) => {
  ...
  const fetchCargas = async () => {
    try {
      ...
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL2}/api/cargas?${params}`);
      const data = await res.json();
      setCargas(data);
      
      // Calcular estadísticas
      const totalCargas = data.length;
      const totalArticulos = data.reduce((sum: number, carga: Carga) => sum + carga.cantidad_cargada, 0);
          ...
          <div className="row g-3 align-items-end">
            <div className="col-md-4">
              ...
              <select
                ...
                value={empresa}
                onChange={(e) => setEmpresa(e.target.value as 'Telecom' | 'Pago Online')}
              >
                    ...
                    <div className="col-md-5">
                      ...
                      >
                        ...
                        {articulos.filter(a => a.disponibles > 0).map(articulo => (
                          <option key={articulo.id} value={articulo.id}>
                            {articulo.simbolo} {articulo.nombre} 
                            <span className="text-success"> (Stock: {articulo.disponibles})</span>
                          </option>
                        ))}
                      </select>
                    </div>
                ...
                <tbody>
                  {cargas.map(carga => (
                    <tr key={carga.id}>
                      <td>
                        <Calendar size={14} className="me-1" />
                        {new Date(carga.fecha_carga).toLocaleString()}
                      </td>
                      <td>
                        <div>
                      ...
                      <td>
                        <div className="d-flex align-items-center">
                          <span className="fs-5 me-2">{carga.articulo_simbolo}</span>
                          <span>{carga.articulo_nombre}</span>
                        </div>
                      </td>
                      <td>
                        <span className="badge bg-success fs-6">
                          {carga.cantidad_cargada}
                        </span>
                      </td>
